...
public class CommandHelp {
    ...
    @Command(value = "AI",description = "AI工具（无参数【注意AI效率极低】）")
    ...
    @Command("热点")
    public void hot(MessageEvent event){
        StringBuffer buffer = new StringBuffer();
        buffer.append( "命令格式：命令前缀 命令主语\r\n" +
                "命令前缀："+ CommandConfig.getInstance().getCommandConfigInfo().getCommandPrefix()+"\r\n");
        buffer.append(1).append("看世界：60s看世界").append("\r\n");
        buffer.append(2).append("虎扑步行街").append("（参数【true】 显示详细信息）").append("\r\n");
        buffer.append(2).append("虎扑步行街").append("（参数【true】 显示详细信息）").append("\r\n");

...
public class GlobalFilterInitOrExecute {
    ...
    private GlobalFilterInitOrExecute() {
        GlobalFilterList = FindClass.getResultsToClasses().stream()
        ...
        //打印局部过滤器列表
        FindClass.getResultsToClasses().stream()
                .filter((Predicate<Class<?>>) aClass -> AbstractFilter.class.isAssignableFrom(aClass))

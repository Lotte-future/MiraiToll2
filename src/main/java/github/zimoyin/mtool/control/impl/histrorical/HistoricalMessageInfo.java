...
//        MessagesSet.getInstance().set(messageSource.getFromId(),chain);//一个人一个表
        ...
        for (SingleMessage singleMessage : chain) {
            ...
            } else if (singleMessage instanceof MarketFace || singleMessage instanceof VipFace) buffer.append("[表情]").append(singleMessage);
            ...
            else if (singleMessage instanceof Image) buffer.append("[图片]: ").append(((Image) singleMessage).getImageId());
            else if (singleMessage instanceof Voice) buffer.append("[语音]: ").append (((Voice) singleMessage).getUrl());
            else if (singleMessage instanceof Audio) buffer.append("[语音]: ").append(Arrays.toString(((Audio) singleMessage).getExtraData()));
            else if (singleMessage instanceof FlashImage) buffer.append("[闪照]: ").append(((FlashImage) singleMessage).getImage().getImageId());
